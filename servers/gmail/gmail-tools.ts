...
export interface GmailApi {
  users: {
    drafts: {
      create: (params: { userId: string; requestBody: { message: { raw: string } } }) => Promise<unknown>;
    };
    messages: {
...
export function buildRawMessage({ to, subject, body }: EmailInput): string {
  const headers = [
    `To: ${to}`,
    `Subject: ${subject}`,
    "MIME-Version: 1.0",
...
export async function createDraft(gmail: GmailApi, input: EmailInput): Promise<string> {
...
export async function sendEmail(gmail: GmailApi, input: EmailInput): Promise<string> {
  const raw = buildRawMessage(input);
