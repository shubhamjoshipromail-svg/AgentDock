...
const VETTED_FLOW_TEMPLATES = [
  ...
  },
  {
    ...
    agent: {
      ...
      model: "claude-sonnet-4-6",
      systemPrompt:
        "Before drafting, ask exactly once with this three-field form intent: " + BRIEF_FORM + ". " +
        "After the user responds, use create_draft to prepare a Gmail draft for the USER EMAIL using their audience, tone, and key point. " +
    ...
    for (const template of VETTED_FLOW_TEMPLATES) {
      ...
      for (const key of desiredToolKeys) {
        ...
        await tx.mcpAccessGrant.upsert({
          ...
          update: { agentId: agent.id, ...definition.grant },
          create: { userId, workflowId: workflow.id, agentId: agent.id, mcpServerId, ...definition.grant }
        });
      }
    }
